...
public static class PlayerParserTraceStore
{
    ...
    // Geriye d�n�k uyumluluk: eski �a�r�lar (time, eventName, payload) bekliyorsa �al��maya devam etsin.
    ...
    public static List<(DateTime time, int eventCode, string eventName, string payload)> GetGlobalEntries()
    {
        lock (_lock)
        {
            return _globalEntries.Select(e => (e.Time, e.EventCode, e.EventName, e.Payload)).ToList();
        }
    }
