            ...
            // --- ZINDANLAR (DUNGEONS) ---
            {
                lock (_dataLock)
                {
                    ...
                    foreach (var d in _dungeonBuffer)
                    {
                        ...
                        if (d.EnchantmentLevel == 1) dCol = 0xFFFF8C00; // Blue/Rare
                ...
                if (_showPlayers)
                {
                    lock (_dataLock)
                    {
                        ...
                        foreach (var p in _playersBuffer)
                        {
                            ...
                            if (p.Id == _devHighlightEntityId)
                            {
                                Vector2 tPos = WorldToScreen(center, new Vector2(p.CurrentLerpedX, p.CurrentLerpedY), new Vector2(mainPlayer.CurrentLerpedX, mainPlayer.CurrentLerpedY));
                                drawList.AddCircleFilled(tPos, 20f, 0x4400FFFF); // Yarı saydam sarı zemin
                                drawList.AddCircle(tPos, 20f, 0xFF00FFFF, 32, 3f); // Sarı kalın kenarlık
                            }
