...
namespace Nightwatch
{
    public partial class AlbionOverlay
    {
        private void RenderMobsTab()
        {
            ...
            {
                ImGui.Indent();
                if (ImGui.TreeNodeEx(Lang.Get("Dungeon_Filters") ?? "Dungeon Filters", ImGuiTreeNodeFlags.DefaultOpen))
                {
                    if (ImGui.TreeNodeEx(Lang.Get("Dungeon_Solo") ?? "Solo Dungeons", ImGuiTreeNodeFlags.DefaultOpen))
                    {
                    ...
                    if (ImGui.TreeNodeEx(Lang.Get("Dungeon_Group") ?? "Group Dungeons", ImGuiTreeNodeFlags.DefaultOpen))
                    {
                    ...
                    if (ImGui.TreeNodeEx(Lang.Get("Dungeon_Corrupted") ?? "Corrupted Dungeons", ImGuiTreeNodeFlags.DefaultOpen))
                    {
                        MentalityTheme.AnimatedToggle("Show Corrupted", ref _showCorruptedDungeons);
                        ImGui.TreePop();
                    }
                    if (ImGui.TreeNodeEx(Lang.Get("Dungeon_Hellgate") ?? "Hellgates", ImGuiTreeNodeFlags.DefaultOpen))
                    {
                        MentalityTheme.AnimatedToggle("Show Hellgate", ref _showHellgateDungeons);
                        ImGui.TreePop();
                    }
                    if (ImGui.TreeNodeEx("Avalonian Dungeons", ImGuiTreeNodeFlags.DefaultOpen))
                    {
