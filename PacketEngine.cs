...
namespace Nightwatch
{
    ...
    public class InternalMapHandler : IEventHandler
    {
        ...
        public void OnResponse(ResponseCodes code, Dictionary<byte, object> parameters)
        {
            ...
            // Çok kısa aralıkta gelen çakışık cevapları filtrele
    ...
    public class PacketEngine
    {
        ...
        private bool ValidatePort(ICaptureDevice device, int port, int timeoutSeconds = 10)
        {
            ...
            try
            {
                    ...
                    try
                    {
                        ...
                        if (packet != null && (packet.SourcePort == port || packet.DestinationPort == port))
                        {
                            if (!_port5056Detected)
                            {
                                ...
                                Console.ForegroundColor = ConsoleColor.Green;
                                Console.WriteLine($"[{DateTime.Now:HH:mm:ss}] ? Port {port} aktif! Albion trafiği algılandı!");
                ...
                while (elapsed < timeoutMs && !_port5056Detected)
                {
                    ...
                    Console.Write($"\r[{DateTime.Now:HH:mm:ss}] ? Bekleniyor... ({remainingSeconds}s kaldı)");
                }
                ...
                if (!_port5056Detected)
                {
                    ...
                    Console.WriteLine("   1. Albion Online çalışıyor mu?");
                    Console.WriteLine("   2. Doğru adaptörü seçtin mi?");
            ...
            catch (Exception ex)
            {
                ...
                Console.ForegroundColor = ConsoleColor.Red;
                Console.WriteLine($"[{DateTime.Now:HH:mm:ss}] ? Port kontrol hatası: {ex.Message}");
                Console.ResetColor();
                success = false;
            }
            finally
        ...
        {
            Console.ForegroundColor = ConsoleColor.Cyan;
            Console.WriteLine($"[{DateTime.Now:HH:mm:ss}] ?? Tüm UDP portları taranıyor (auto discover)...");
            Console.ResetColor();
            ...
            // KeÅŸiften önce önceki kalıntı istatistikleri temizleyelim ki canlı sayaç net görünsün.
            ...
            try
            {
                tempHandler = (sender, e) =>
                {
                    try
                    {
                        ...
                        // Photon parse'e düşmeden önce hafif bir eleme
                        ...
                        {
                            // parse başarısızsa skor vermiyoruz
                        }
                    }
                    catch
                    {
        ...
        public void Start()
        {
                ...
                foreach (var dev in devices)
                {
                    if (dev.Description == savedAdapter)
                    {
                        try
                        {
                            ...
                            UIConsole.Log($"[Device] Kayıtlı özel adaptör aktif: {dev.Description}", LogLevel.Info);
                            break;
                        }
                        catch (Exception ex) { UIConsole.Log($"Adaptör açılamadı: {ex.Message}", LogLevel.Error); }
                    }
                }
            }
            ...
            UIConsole.Log($"PacketEngine aktif. Dinlenen adaptör sayısı: {opened}", LogLevel.Info);
        }
