...
namespace Nightwatch
{
    ...
    public partial class AlbionOverlay : Overlay, IDisposable
    {
        ...
        public AlbionOverlay(GameStateManager manager, bool isRunningAsAdmin) : base("Nightwatch Overlay")
        {
            ...
            #region System Tray
            // --- SYSTEM TRAY (SAÄz ALT KÃ–ÅzE Ä°KONU) THREAD KURULUMU ---
            new System.Threading.Thread(() =>
            {
                try
                {
                    ...
                    // Ã‡ift tÄ±klayÄ±nca menÃ¼yÃ¼ geri aÃ§ar
                    _trayIcon.MouseDoubleClick += (s, e) => { _hideSettingsWindow = false; };

                    // SaÄŸ tÄ±k menÃ¼sÃ¼ (Profesyonel dokunuÅŸ)
                    ...
                    // EN Ã–NEMLÄ° KISIM: Ä°konun tÄ±klamalarÄ± algÄ±lamasÄ± iÃ§in Windows Mesaj DÃ¶ngÃ¼sÃ¼nÃ¼ baÅŸlatÄ±r!
            ...
            // --- OTOMATÄ°K CONFIG YÃœKLEME (Autoconfig.txt) ---
            string defaultTxtPath = System.IO.Path.Combine(baseDir, "Assets", "Autoconfig.txt");
            string configToLoad = "Varsayilan"; // Dosya yoksa bunu yÃ¼kler
            ...
            try
            {
                if (File.Exists(defaultTxtPath))
                {
                    ...
                    if (!string.IsNullOrEmpty(content))
                    {
                        ...
                        // GÃœVENLÄ°K: Path traversal Ã¶nlemi â€” sadece dosya adÄ± geÃ§ebilir
