...
namespace Nightwatch.UserControls
{
    ...
    public static class MentalityTheme
    {
        ...
        public static uint GetRainbowColor(float offset = 0f, float speed = 1f, float alpha = 1f)
        {
            float t = (float)ImGui.GetTime() * speed + offset;
            float r = (float)(Math.Sin(t) * 0.5 + 0.5);
            float g = (float)(Math.Sin(t + 2.094) * 0.5 + 0.5);
            float b = (float)(Math.Sin(t + 4.188) * 0.5 + 0.5);
            return ImGui.ColorConvertFloat4ToU32(new Vector4(r, g, b, alpha));
        }
        ...
        public static bool TreeNode(string label, bool defaultOpen = false)
        {
            ...
            if (defaultOpen) flags |= ImGuiTreeNodeFlags.None;
