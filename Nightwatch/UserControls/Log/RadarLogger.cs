...
namespace Nightwatch
{
    ...
    public static class RadarLogger
    {
        ...
        static RadarLogger()
        {
            ...
            {
                Nightwatch.UIConsole.Log($"Error Code : 67 | {ex.Message}", Nightwatch.LogLevel.Error);
                Nightwatch.UIConsole.Log(string.Format(Lang.Get("Log_DirError"), ex.Message), Nightwatch.LogLevel.Error);
            }

            string fileName = $"{DateTime.Now:dd-MM-yyyy_HH-mm}-Log.json";
            _logFilePath = Path.Combine(dir, fileName);

            Nightwatch.UIConsole.Log(string.Format(Lang.Get("Log_PathInfo"), _logFilePath), LogLevel.Warning);
        ...
        public static void LogPlayer(string mapId, string name, int ip, string weapon, string head, string armor, string shoes, string cape)
        {
            ...
            lock (_lock)
            {
                ...
                Nightwatch.UIConsole.Log(string.Format(Lang.Get("Log_PlayerSaved"), name), LogLevel.Info);
        ...
        public static void LogResource(string mapId, string type, string tier, string amount, float posX, float posY)
        {
            ...
            lock (_lock)
            {
                ...
                Nightwatch.UIConsole.Log(string.Format(Lang.Get("Log_ResourceSaved"), type, tier), LogLevel.Info);
        ...
        private static void CheckAndAutoSave()
        {
            ...
            {
                Nightwatch.UIConsole.Log(Lang.Get("Log_AutoSave"), LogLevel.Warning);
                SaveFile();
        ...
        public static void SaveFile()
        {
            try
            {
                lock (_lock)
                {
                    ...
                    // FileShare.ReadWrite › başka process aynı anda erişse bile kilitlenmiyor
                    using var fs = new FileStream(_logFilePath, FileMode.Create, FileAccess.Write, FileShare.ReadWrite);
                    using var sw = new StreamWriter(fs);
                    sw.Write(json);

                    Nightwatch.UIConsole.Log(string.Format(Lang.Get("Log_SaveSuccess"), _currentLog.Players.Count, _currentLog.Resources.Count), LogLevel.Success);
                }
            }
            catch (Exception ex)
            {
                Nightwatch.UIConsole.Log($"Error Code : 68 | {ex.Message}", Nightwatch.LogLevel.Error);
                Nightwatch.UIConsole.Log(string.Format(Lang.Get("Log_SaveError"), ex.Message), Nightwatch.LogLevel.Error);
            }
        }
    }
}
