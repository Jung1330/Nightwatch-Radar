...
namespace Nightwatch
{
    public partial class AlbionOverlay
    {
            ...
            bool d11, bool d12, bool d13, bool d14, bool d15)
        {
            ...
            if (d15)
            {
                // p1 list [0,1] genelde byte list olabilir; parse stringinden ilk iki byte'ý dene
                float lx = bytes[0];
                float ly = bytes[1];
        ...
        private void ApplyModernStyle()
        {
            ...
            // ==========================================
            // 2. ARKA PLAN VE BAÞLIK EÞÝTLEMESÝ (Kusursuz Görünüm)
            // ==========================================
            // Senin verdiðin RGB: 1, 2, 3 (Ýçi ve baþlýðý ayný renk yapýyoruz ki çizgi olmasýn)
            Vector4 mainBgColor = new Vector4(1f / 255f, 2f / 255f, 3f / 255f, 0.98f); // 0.98f hafif saydamlýk
            ...
            // BAÞLIK ÇUBUÐUNU (TASI BENI YAZAN YERÝ) GÝZLEYEN SÝHÝRLÝ KISIM:
            ...
            // Çerçeve (Border) rengini ana renkten çok hafif daha açýk yapýyoruz ki tatlý bir sýnýrý olsun
            ...
            // ==========================================
            // 3. ANA VURGU RENGÝ (Accent Color)
            // ==========================================
            ...
            // Sekme (Tab) renkleri - Midnight mor (seçili daha açýk)
            ...
            // ImGuiCol enumunda TabActive yoksa index ile güvenli þekilde ayarla
                                ...
                                if (ImGui.BeginTabItem("Crown"))
                                {
                                    ...
                                    if (crownRefreshNeeded || _cachedCrownResults == null)
                                    {
                                        _cachedCrownResults = _mobDatabase.Where(x =>
                                        {
                                            ...
                                            // Arama kutusu doluyken tüm veritabanýnda ara (boss kýsýtý kaldýrýlýr)
                                            ...
                                            // Arama boþken eski davranýþ: yalnýzca boss benzeri kayýtlar
                                            return isBoss;
                                        }).ToList();
...
*/
                                ...
                                if ((DateTime.Now - _lastAutoRawDumpTime).TotalSeconds >= 0.5)
                                {
                                    ...
                                    lock (_dataLock)
                                    {
                                        ...
                                        */
                                        ...
                                        // 2. Diðer oyuncularý yazdýr (ilk 20, mesafe ile)
                            ...
                            if (ImGui.BeginChild("ParserDump", new Vector2(0, 0), ImGuiChildFlags.Borders))
                            {
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        // En yeni en altta kalacak þekilde sýrala
                        ...
                        // YANLIÞLIKLA SÝLÝNEN KOD BURASI: Sekme çubuðunu (TabBar) kapatýyoruz!
                        ImGui.EndTabBar();
