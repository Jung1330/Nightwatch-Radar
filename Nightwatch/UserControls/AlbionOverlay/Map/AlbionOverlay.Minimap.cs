    ...
    public partial class AlbionOverlay
    {
        ...
        private void DrawRadar(ImDrawListPtr drawList, Vector2 winPos, Vector2 winSize, Player mainPlayer)
        {
            ...
            if (mainPlayer != null)
            {
                ...
                if (_showPlayers)
                {
                    lock (_dataLock)
                    {
                        ...
                        foreach (var p in _playersBuffer)
                        {
                            ...
                            // Son pozisyondan belirgin farklýysa ekle (0.3 dünya birimi)
                            ...
                            // Trail noktalarýný soluk çiz (eski › yeni, alfa azalýyor)
                ...
                lock (_dataLock)
                {
                    ...
                    foreach (var m in _mobBuffer)
                    {
                        ...
                        {
                            // Kesin kural: isim kaynaðý TypeId metadata (deterministik)
                        ...
                        // --- ÝSÝM ÝÇÝNDEKÝ GEREKSÝZ TAKILARI SÝLME ---
                        if (displayName != "Unknown")
                        {
                            ...
                            // Eðer geriye hiçbir þey kalmadýysa Unknown yap (Çizim aþamasýndaki giþe bunu tamamen gizleyecektir)
                        ...
                        // Ã–ZEL Ä°KONLAR (Sniffers, Bosses, Crystals, Drones)
                        ...
                        // 908 ile 923 arasÄ±ndaki tÃ¼m ID'ler Avalon Treasure Minion'dur
                        else if (m.TypeId >= 908 && m.TypeId <= 923)
                        {
                            specificIcon = System.IO.Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Assets", "Resources", "AVALONMINIONCHEST.png");
                        }
                        ...
                        if ((isPriority || isTrackerCustom) && _showEnemyMobs)
                        {
                            ...
                            // Otomatik renk: her kategori farklÄ± â€” Custom=Cyan, Priority=Turuncu, her ikisi=SarÄ±
                            uint mobLaserCol;
                            ...
                            DrawImageOrDot(drawList, center, mainPlayer, m.CurrentLerpedX, m.CurrentLerpedY, iconToUse, COL_SPECIAL, displayName, radiusLimit, _globalIconSize + 10, doEdgeClamp, mobLaserCol);
                            continue;
                        }
                        ...
                        // Kesin kural: canlý kaynak sýnýflamasý yalnýzca TypeId tabanlý metadata ile yapýlýr.
                        // Böylece Recruit vb. normal moblar isimden yanlýþlýkla "Ore/Fiber Creature" etiketine düþmez.
                        ...
                        bool isLivingResource = (mobCategory != HarvestableCategory.None);
                        ...
                        {
                            // Sadece Hidden Chest çiz (diðer chest rarity/normal chest kapalý)
                        ...
                        if (isLivingResource)
                        {
                            ...
                            {
                                int rawNameTier = string.IsNullOrEmpty(m.Name) ? 0 : ParseTier(m.Name);
                                int metaTier = resolvedLivingTier > 0 ? resolvedLivingTier : (info?.Tier ?? 0);

                                // Oyun içi etikete daha yakýn görünüm için adý önceliklendir.
                                // Ýsimde tier yoksa metadata fallback kullan.
                                int tier = rawNameTier > 0 ? rawNameTier : metaTier;

                                if (tier == 0) tier = ParseTier(displayName);
                                if (tier > 1)
                                {
                                    ...
                                    {
                                        string resName = _resourceMobNames.TryGetValue(mobCategory, out var n) ? n : mobCategory.ToString();
                                        string label = (enchant > 0) ? $"T{tier}.{enchant} {resName}" : $"T{tier} {resName}";
                                        if (!_resourceTrackerOnlyMode)
                                        {
                                            DrawRadarDot(drawList, center, mainPlayer, m.CurrentLerpedX, m.CurrentLerpedY, 0xFF00FF00, label, radiusLimit, false, 4.0f, _trackerEnableResources, 0xCC66FF66, showOffScreenArrow: true);
                                            renderedAsResource = true;
                                        }
                                        else if (_trackerEnableResources)
                                        {
                                            DrawRadarDot(drawList, center, mainPlayer, m.CurrentLerpedX, m.CurrentLerpedY, 0x00000000, label, radiusLimit, false, 0.1f, true, 0xCC66FF66, showOffScreenArrow: true, hideMarker: true);
                                            renderedAsResource = true;
                                        }
                                    }
                                }
                            }
                            // Filtrede seçili deðilse normal düþman mob akýþýna düþsün (kýrmýzý nokta).
                        ...
                        if (_showEnemyMobs)
                        {
                            ...
                            if (isBigBoss)
                            {
                                if (_showBosses)
                                {
                                    ...
                                    DrawImageOrDot(drawList, center, mainPlayer, m.CurrentLerpedX, m.CurrentLerpedY, bossIcon, COL_GOLD, label, radiusLimit, bossSize, _trackerEnableVipMobs, bossLaser, showOffScreenArrow: true);
                                }
                            }
                            else { if (_showNormalMobs) DrawRadarDot(drawList, center, mainPlayer, m.CurrentLerpedX, m.CurrentLerpedY, COL_RED, label, radiusLimit, false, 3.0f, _trackerEnableNormalMobs, 0xCC4466FF); }
                        }
                ...
                if (_showResources)
                {
                    lock (_dataLock)
                    {
                        ...
                        foreach (var h in _harvestBuffer)
                        {
                            ...
                            // --- DÃœZELTME BURADA: Ä°Ã§i tamamen boÅŸalanÄ± (0) radardan direkt GÄ°ZLE ---
                            if (h.Count <= 0) continue;

                            // --- RENDER DISTANCE KONTROLÃœ (FPS OPTÝMÝZASYONLU) ---
                            ...
                            // Math.Sqrt kullanmak yerine karelerin toplamýný, renderDistance'ýn karesiyle kýyaslýyoruz mq
                            ...
                            {
                                int tier = h.Tier; int enchant = h.EnchantmentLevel;
                                int tierIndex = Math.Max(0, Math.Min(tier - 1, 7)); int enchantIndex = Math.Min(enchant, 3);

                                if (_resourceFilters[cat][tierIndex, enchantIndex])
                                {
                                    string label = $"T{tier}.{enchant} {cat} ({h.Count})";

                                    // Tier + enchant kombinasyonuna Ã¶zgÃ¼ renk (T5.0â‰ T5.1â‰ T5.2â‰ T5.3)
                                    uint tCol = GetTierEnchantColor(tier, enchant);
                                    uint resLaserCol = tCol;

                                    bool iconDrawn = false;
                                    if (!_resourceTrackerOnlyMode && _showResourceIcons)
                                    {
                                        ...
                                        if (iconExists)
                                        {
                                            // Eski sabit mor (0xFFFF00FF) yerine tCol veriyoruz
                                            DrawImageOrDot(drawList, center, mainPlayer, h.CurrentLerpedX, h.CurrentLerpedY, imgPath, tCol, label, radiusLimit, _globalIconSize, _trackerEnableResources, resLaserCol);
                                            iconDrawn = true;
                                        }
                                    }

                                    if (!_resourceTrackerOnlyMode && !iconDrawn)
                                    {
                                        // Eski sabit mor (0xFFFF00FF) yerine tCol veriyoruz
                                        DrawRadarDot(drawList, center, mainPlayer, h.CurrentLerpedX, h.CurrentLerpedY, tCol, label, radiusLimit, false, 4.0f, _trackerEnableResources, resLaserCol);
                                    }

                                    if (_resourceTrackerOnlyMode && _trackerEnableResources)
                                    {
                                        DrawRadarDot(drawList, center, mainPlayer, h.CurrentLerpedX, h.CurrentLerpedY, 0x00000000, label, radiusLimit, false, 0.1f, true, resLaserCol, showOffScreenArrow: true, hideMarker: true);
                                    }
            ...
            if (ImGui.IsWindowHovered() && ImGui.IsMouseClicked(ImGuiMouseButton.Right) && !ImGui.IsAnyItemHovered() && mainPlayer != null)
            {
                ...
                {
                    _waypoint = null; // Radar dýþýna sað týklama › waypoint kaldýr
                }
            }
        }
        #endregion
        ...
        private void DrawImageOrDot(ImDrawListPtr dl, Vector2 center, Player p, float tx, float ty, string imgPath, uint fallbackCol, string lbl, float lim, float size, bool edgeClamp = false, uint laserCol = 0, bool showOffScreenArrow = false)
        {
            ...
            if (edgeClamp)
            {
                ...
                if (_invertX) ldx = -ldx;
                if (_invertY) ldy = -ldy;
                // Gerçek Ýzometrik 45 derece Kamera Açýsý (Minimap'ten baðýmsýz, oyuna tam kilitli)
                ...
                if (laserVec.LengthSquared() > 0.0001f)
                {
                    ...
                    DrawCompassIndicator(fgDrawList, screenCenter, laserNorm, targetOnScreen, finalLaserCol, lbl, _cachedPrimaryScreenW, _cachedPrimaryScreenH);
                }
            }
            // -----------------------------------------------------------
            ...
            if (IsImageExistsCached(imgPath))
            {
                ...
                catch (Exception ex)
                {
                    ...
                    if (_debugConsoleLog) Log($"[HATA] Ýkon Çizilemedi: {ex.Message}", LogLevel.Warning);
                    iconDrawn = false;
                }
            }
        ...
        private void DrawRadarDot(ImDrawListPtr dl, Vector2 center, Player p, float tx, float ty, uint col, string lbl, float lim, bool isSquare = false, float size = 4.0f, bool edgeClamp = false, uint laserCol = 0, bool showOffScreenArrow = false, bool hideMarker = false)
        {
            Vector2 final = WorldToScreen(center, new Vector2(tx, ty), new Vector2(p.CurrentLerpedX, p.CurrentLerpedY));
            Vector2 dir = final - center;
            float dist = dir.Length();
        ...
        // Off-screen ok: radar çemberinin kenarýnda dolgu üçgen ok çizer
        // lbl verilirse etiketi okun ARKASINDA (merkeze doðru) – okun ucuyla çakýþmaz
        private void DrawOffScreenArrow(ImDrawListPtr dl, Vector2 center, float radius, Vector2 normalizedDir, uint color, string lbl = "")
        {
            ...
            // Etiket: okun tabanýndan merkeze doðru (radar içinde, okla çakýþmaz)
            if (!string.IsNullOrEmpty(lbl))
            {
                var ts = ImGui.CalcTextSize(lbl);
                float inset = arrowSize * 1.8f + ts.Y + 10f;  // ok tabanýndan içeriye mesafe
                Vector2 lblAnchor = tip - normalizedDir * inset - new Vector2(ts.X / 2f, ts.Y / 2f);
                // Radar dairesinin içinde kalmasýný garantile
        ...
        // Compass HUD Indicator â€” merkez Ã§izgisi yerine ring Ã¼zerinde yÃ¶n oku + stem + etiket.
        // Birden fazla hedef olduÄŸunda ortada oluÅŸan Ã§izgi yÄ±ÄŸÄ±lmasÄ±nÄ± tamamen ortadan kaldÄ±rÄ±r.
        // Snapline (Düz Çizgi) Tracker Sistemi
        // Yön Gösterici (Sabit Uzunluklu Çizgi) Tracker Sistemi
        // Hibrit Çizgi (Minimum Uzunluk + Dinamik Mesafe) Tracker Sistemi
        // Yön Gösterici (Sabit Uzunluklu Çizgi) Tracker Sistemi
        private void DrawCompassIndicator(ImDrawListPtr dl, Vector2 center, Vector2 dir,
            Vector2 targetPos, uint accentCol, string lbl, float scrW, float scrH)
        {
            // Karakterin üstünü kapatmamasý için merkezden biraz boþluk býrakarak baþlatýyoruz
            float startGap = 40f;
            Vector2 lineStart = center + dir * startGap;

            // --- DÝNAMÝK MESAFE (DISTANCE CLAMPING) AMELÝYATI ---
            // Çizginin ucu artýk sabit 300 piksel deðil, hedefin gerçek ekran koordinatýnda bitiyor!
            Vector2 lineEnd = targetPos;

            // Aþýrý sapmalarda çizginin ekranýn köþelerinden fýrlamasýný engelle
            ...
            // Eðer hedef karakterin çok dibindeyse (startGap'ten bile yakýnsa) çizginin ters dönmemesi için:
            if (Vector2.Distance(center, lineEnd) <= startGap)
            {
                lineStart = lineEnd; // Dibindeyken çizgi kaybolur, sadece hedef noktasý kalýr
            }

            // Dümdüz, hedefe tam yapýþan dinamik çizgi
            dl.AddLine(lineStart, lineEnd, accentCol, 1.5f);

            // Çizginin ucuna küçük bir hedef noktasý
            dl.AddCircleFilled(lineEnd, 3f, accentCol);

            // Etiket (Ýsim) çizimi
            if (!string.IsNullOrEmpty(lbl))
            {
                // KESÝN ÇÖZÜM: Ýsim "Mob", "Enemy" veya "Unknown" ise asla ekrana çizme!
                string uLbl = lbl.ToUpperInvariant();
                if (uLbl != "TN" && uLbl != "MOB" && uLbl != "ENEMY" && uLbl != "UNKNOWN" && !uLbl.StartsWith("ID:"))
                {
                    var ts = ImGui.CalcTextSize(lbl);

                    // Etiketi çizginin ucunun hemen altýna ortala
                    Vector2 textAnchor = lineEnd + new Vector2(-ts.X / 2f, 6f);

                    // Ekran dýþýna taþmasýný engelle
