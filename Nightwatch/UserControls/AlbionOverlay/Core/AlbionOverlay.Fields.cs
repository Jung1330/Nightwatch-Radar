...
namespace Nightwatch
{
    public partial class AlbionOverlay
    {
        ...
        // --- SYSTEM TRAY (SA�z ALT İKON) DE�zİ�zKENİ ---
        private System.Windows.Forms.NotifyIcon _trayIcon;
        private System.Windows.Forms.ContextMenuStrip _trayContextMenu;

        // GDI ikon handle'ları — Dispose'da serbest bırakılacak
        ...
        // Kart slotlar�: null = bo� | int = playerId
        ...
        private float _trackerStartGap = 0; // Karakterin içinden geçmemesi için boşluk
        private float _trackerScaleX = 7.0f;          // Piksel/birim — Dünya X ekseni (sağ/sol)
        private float _trackerScaleY = 7.0f;          // Piksel/birim — Dünya Y ekseni (ileri/geri)
        private float _trackerAngleOffset = 0f;       // Açı ince ayarı (derece, -45 → +45)
        ...
        private string[] _tabs = new string[6]; // Sabit 6 elemanlı dizi (Çökmeyi engeller)
        ...
        private int _selectedLangIndex = 0; // 0 = TR, 1 = EN
        private string[] _languages = { "T�rk�e (TR)", "English (EN)" };
        ...
        // --- TÜRKÇE FONT İÇİN RAM SABİTLEYİCİ DE�zİ�zKENLER ---
        private GCHandle _trRangesHandle;
        // Tüm Türkçe karakterleri tek aralık olarak birleştirdik (0x011E-0x015F kapsar: �z,ğ,İ,ı,�z,ş + Ç,ç,Ö,ö,Ü,ü zaten 0x00C7-0x00FC arasında)
        private static ushort[] _trRanges = new ushort[] {
            0x0020, 0x00FF, // İngilizce, temel semboller + Ç,ç,Ö,ö,Ü,ü
            0x011E, 0x015F, // �z,ğ,İ,ı,�z,ş (tek birleşik aralık)
        ...
        // Yakla�an d��man y�n g�stergesi (ekran kenar� ok)
        private bool _showDangerCompass = true;

        // Performans: Random sınıf seviyesinde bir kez oluşturulur
        private static readonly Random _rng = new Random();

        // Performans: GetSystemMetrics çağrıları cache'lenir (her frame sistem ça�rısı yapılmaz)
