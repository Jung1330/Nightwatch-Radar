...
namespace Nightwatch
{
    ...
    {
        #region Veritabanı Yükleyicileri (Database Loaders)
        ...
        private void CheckAndLoadDatabase()
        {
            ...
            string targetFileName = $"mobs_{currentLang}_min.json";
            ...
            else
            {
                ...
                string fallbackPath = System.IO.Path.Combine(baseDir, "Assets", "Helper", "mobs_EN_min.json");
        ...
        private void LoadMobDatabase(string path)
        {
            ...
            {
                Log(string.Format(Lang.Get("Error_MobDbLoad") ?? "[HATA] Mob DB Yüklenemedi: {0}", ex.Message), Nightwatch.LogLevel.Error);
        ...
        private void LoadZonesDatabase()
        {
            ...
            if (File.Exists(path))
            {
                ...
                {
                    Nightwatch.UIConsole.Log($"[HATA] Error Code: " + ex.Message, Nightwatch.LogLevel.Error);
                    Log(string.Format(Lang.Get("ZonesError"), ex.Message), Nightwatch.LogLevel.Error);
                }
            }
        }
        #endregion
